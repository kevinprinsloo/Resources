...
%       Modifed: 26 Apr  2015 (Change error massages  "Numch*1"
%                               current MakeAsymCmpFiltersV2.mexmaci64 only produce 100ch filter)
%
%   function [SigOut,ACFstatus] = ACFilterBank(ACFcoef,ACFstatus,SigIn,SwOrdr);
%	INPUT : ACFcoef: coef from MakeAsymCmpFiltersV2
...
%end;
[NumChSig LenSig] = size(SigIn);
if LenSig ~= 1 
   error('Input Signal should be NumCh*1 vector (1 sample time-slice)');
end;
if NumChSig ~= ACFstatus.NumCh
   error(['NumChSig (' int2str(NumChSig) ') ~= ACFstatus.NumCh (' int2str(ACFstatus.NumCh) ')']);
end;
